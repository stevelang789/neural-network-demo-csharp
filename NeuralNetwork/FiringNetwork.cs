...
namespace NeuralNetwork
{
    public class FiringNetwork
    {
        ...
        public FiringNetwork(Network net)
        {
            _net = net;
            _neurons = _net.Neurons.Select(layer => layer.Select(n => new FiringNeuron(n)).ToArray()).ToArray();
            _neuronsWithLayersReversed = _neurons.Reverse().ToArray();
        }

        public FiringNeuron[] OutputLayer => _neurons[_neurons.Length - 1];
        ...
        public void FeedForward(double[] inputValues)
        {
            ...
            foreach (var layer in _neurons)
            {
                ...
                _net.Activators[i++].ComputeOutputs(layer);
